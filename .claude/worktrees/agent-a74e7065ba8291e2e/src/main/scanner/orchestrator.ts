  ...
  async startScan(scanId: string): Promise<void> {
    ...
    try {
      ...
      const PROFILE_MODULES: Record<string, string[]> = {
        ...
        deep: ['recon', 'headers', 'tls-ssl', 'sql-injection', 'xss', 'command-injection', 'ssti', 'nosql-injection', 'cors', 'open-redirect', 'csrf', 'clickjacking', 'ssrf', 'xxe', 'jwt', 'access-control', 'crlf-injection', 'file-inclusion', 'host-header', 'cookie-analysis', 'js-analysis'],
      ...
      if (config.modules && config.modules.length > 0) {
        ...
        const profileModules = PROFILE_MODULES[config.profile || 'standard'];
        enabledModules = profileModules && profileModules.length > 0
          ? ALL_MODULES.filter(m => profileModules.includes(m.id))
          : ALL_MODULES;
      }
