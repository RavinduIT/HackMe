  ...
  async scan(url: string, endpoints: EndpointInfo[], client: HttpClient): Promise<ModuleFinding[]> {
    ...
    for (const ep of endpoints) {
      for (const param of ep.params) {
        ...
        // Baseline check: fetch with benign value to avoid false positives
        const baselineRes = await this.inject(ep, param, 'hm_ssti_baseline', client);

        for (const { payload, expected, engines } of SSTI_PROBES) {
          const res = await this.inject(ep, param, payload, client);
          if (res.status === 0) continue;

          if (res.body.includes(expected) && !res.body.includes(payload) && !baselineRes.body.includes(expected)) {
            findings.push({
              severity: 'critical',
