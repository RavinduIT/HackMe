...
export class CookieAnalysisModule implements ScanModuleInterface {
  ...
  async scan(url: string, endpoints: EndpointInfo[], client: HttpClient): Promise<ModuleFinding[]> {
    ...
    // Session fixation check: make two requests, compare session cookies
    ...
    for (const raw of setCookieHeaders) {
      ...
      for (const raw2 of cookies2) {
        const p2 = this.parseCookie(raw2);
        if (p1.name === p2.name && p1.value === p2.value && p1.value.length > 4) {
          findings.push({
            severity: 'medium',
            title: 'Possible Session Fixation',
            description: `Cookie "${p1.name}" has the same value across two separate unauthenticated requests, suggesting the session ID may be fixed rather than regenerated per session.`,
            url,
