...
export class TlsSslModule implements ScanModuleInterface {
  ...
  async scan(url: string, endpoints: EndpointInfo[], client: HttpClient): Promise<ModuleFinding[]> {
    ...
    try {
      ...
      // Probe for weak TLS versions accepted alongside strong ones
      const weakVersions: { version: string; maxVersion: 'TLSv1' | 'TLSv1.1' }[] = [
        { version: 'TLSv1', maxVersion: 'TLSv1' },
        { version: 'TLSv1.1', maxVersion: 'TLSv1.1' },
      ];

      for (const { version, maxVersion } of weakVersions) {
        try {
          const accepted = await this.probeWeakTls(hostname, port, maxVersion);
          if (accepted) {
            findings.push({
              severity: 'high',
              title: `Server Accepts Deprecated TLS Version: ${version}`,
              description: `The server accepts connections using ${version}, which has known vulnerabilities and is deprecated. Even though the server may prefer a stronger version, accepting ${version} allows downgrade attacks.`,
              url,
              remediation: `Disable ${version} in the server TLS configuration. Only allow TLS 1.2 and 1.3.`,
              cwe_id: 'CWE-327', owasp_category: 'A02:2021', cvss_score: 7.4,
              evidence: { weak_version: version, accepted: true },
            });
          }
        } catch {}
      }
  ...
  private probeWeakTls(hostname: string, port: number, maxVersion: 'TLSv1' | 'TLSv1.1'): Promise<boolean> {
    return new Promise((resolve) => {
      ...
      const socket = tls.connect(
        { host: hostname, port, rejectUnauthorized: false, servername: hostname, maxVersion },
        () => {
