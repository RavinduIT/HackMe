...
export class RequestSmugglingModule implements ScanModuleInterface {
  ...
  private connectSocket(host: string, port: number, isHttps: boolean): Promise<net.Socket> {
    return new Promise((resolve, reject) => {
      const rawSocket = net.connect({ host, port }, () => {
        if (isHttps) {
          const tlsSocket = tls.connect({ host, port, rejectUnauthorized: false, socket: rawSocket, servername: host }, () => {
            resolve(tlsSocket);
          });
  ...
  private timingProbe(host: string, port: number, isHttps: boolean, _variant: string, payload: string): Promise<{ suspicious: boolean; delay: number; baseline: number }> {
    return new Promise((resolve) => {
      const baselineStart = Date.now();
      const baselineReq = `GET / HTTP/1.1\r\nHost: ${host}\r\nConnection: close\r\n\r\n`;

      this.connectSocket(host, port, isHttps).then((baseSocket) => {
        ...
        baseSocket.on('data', () => {
          if (!baselineDone) {
            ...
            this.connectSocket(host, port, isHttps).then((probeSocket) => {
