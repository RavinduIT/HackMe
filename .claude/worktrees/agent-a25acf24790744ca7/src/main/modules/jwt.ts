...
export class JwtModule implements ScanModuleInterface {
  ...
  async scan(url: string, endpoints: EndpointInfo[], client: HttpClient): Promise<ModuleFinding[]> {
    ...
    for (const jwt of allJwts) {
      try {
        ...
        // Try weak secrets brute-force for HMAC tokens
        if (header.alg === 'HS256' || header.alg === 'HS384' || header.alg === 'HS512') {
          const weakFindings = await this.tryWeakSecrets(jwt, client);
          findings.push(...weakFindings);
        }
      } catch {}
    }
  ...
  private async tryWeakSecrets(token: string, _client: HttpClient): Promise<ModuleFinding[]> {
    const findings: ModuleFinding[] = [];
    ...
    for (const secret of WEAK_SECRETS) {
      ...
      if (computed === signatureB64) {
        findings.push({
          ...
          description: `The JWT is signed with the weak secret "${secret}". An attacker who knows this secret can forge arbitrary tokens, impersonate any user, and escalate privileges. The secret was discovered via brute-force of common weak secrets.`,
          url: '',
          remediation: 'Use a cryptographically random secret of at least 256 bits. Rotate the current secret immediately and invalidate all existing tokens.',
          cwe_id: 'CWE-326', owasp_category: 'A02:2021', cvss_score: 9.8,
          evidence: { algorithm: header.alg, weak_secret: secret, token_preview: token.slice(0, 50) + '...' },
        });
        break;
      }
    }
