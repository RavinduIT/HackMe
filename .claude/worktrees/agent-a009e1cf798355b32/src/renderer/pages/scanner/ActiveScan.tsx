...
export default function ActiveScan() {
  ...
  useEffect(() => {
    if (!scanId) return;
    loadFindings();
    const u1 = window.hackme.onScanProgress((d: any) => { if (d.scanId === scanId) setProgress(d); });
  ...
  async function stop() { if (scanId) await window.hackme.stopScan(scanId); }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 900 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          <h1 className="pg-title">{done ? 'Scan Complete' : 'Scanning'}</h1>
          <p className="pg-sub">{done ? `${findings.length} vulnerabilities found` : progress.module || 'Initializing...'}</p>
        </div>
        {done ? (
