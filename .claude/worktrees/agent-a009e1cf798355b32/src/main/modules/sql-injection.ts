...
export class SqlInjectionModule implements ScanModuleInterface {
  ...
  async scan(url: string, endpoints: EndpointInfo[], client: HttpClient): Promise<ModuleFinding[]> {
    ...
    // Test path parameters (numeric IDs in URLs)
    for (const ep of endpoints) {
      const pathParams = extractPathParams(ep.url);
      for (const pp of pathParams) {
        const testUrl = replacePathSegment(ep.url, pp.paramIndex, "' OR '1'='1");
        try {
          ...
          for (const { db, patterns } of ERROR_PATTERNS) {
            for (const pat of patterns) {
              if (pat.test(res.body)) {
                findings.push({
                  severity: 'critical',
                  title: `SQL Injection in URL Path (${db})`,
                  description: `Path segment at position ${pp.paramIndex} (original value "${pp.original}") in ${ep.url} is vulnerable to SQL injection. Database error from ${db} detected in response.`,
                  url: ep.url,
                  parameter: `path[${pp.paramIndex}]`,
                  evidence: { payload: "' OR '1'='1", db, original: pp.original },
                  remediation: 'Use parameterized queries. Never concatenate URL path segments into SQL queries.',
