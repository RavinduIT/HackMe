...
export class ScanOrchestrator {
  ...
  async startScan(scanId: string): Promise<void> {
    ...
    try {
      ...
      this.emitProgress(scanId, { module: wasCancelled ? 'Cancelled' : 'Complete', percent: 100, requests: client.getRequestCount() });
    } catch (err) {
      this.db.prepare("UPDATE scans SET status = 'failed', completed_at = CURRENT_TIMESTAMP WHERE id = ?").run(scanId);
  ...
  private saveFinding(scanId: string, moduleId: string, f: ModuleFinding) {
    // Deduplicate: skip if a finding with the same title and URL already exists for this scan
    const existing = this.db.prepare(
      'SELECT id FROM findings WHERE scan_id = ? AND title = ? AND url = ?'
    ).get(scanId, f.title, f.url);
    if (existing) return;
