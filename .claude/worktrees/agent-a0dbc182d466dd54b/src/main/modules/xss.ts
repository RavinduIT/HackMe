...
const PAYLOADS = [
  ...
  { payload: `{{7*7}}`, context: 'Client-side template injection', severity: 'medium' as const },
];
    ...
    for (const ep of endpoints) {
      ...
      for (const param of ep.params) {
        ...
        for (const { payload, context, severity } of PAYLOADS) {
          ...
          if (payload === '{{7*7}}' && res.body.includes('49') && !canaryRes.body.includes('49')) {
            findings.push({
              severity: 'critical',
              title: 'Client-Side Template Injection',
              description: `Parameter "${param}" is interpreted by a client-side template engine. The expression {{7*7}} was evaluated to 49, confirming template injection. This can escalate to XSS via constructor.constructor payloads or to server-side template injection if processed server-side.`,
              url: ep.url, parameter: param,
              ...
              evidence: { payload: '{{7*7}}', result: '49', parameter: param },
            });
            break;
          }
