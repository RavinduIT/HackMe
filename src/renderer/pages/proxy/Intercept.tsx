...
export default function Intercept() {
  ...
  async function refresh() {
    ...
    try {
      ...
      setProxyCount(Array.isArray(pl) ? pl.length : 0);
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            {proxyOn && proxyCount > 0 && (
              <span className="font-mono" style={{ fontSize: 9, color: 'var(--med)', letterSpacing: '0.04em' }}>
                ROUTING VIA {proxyCount} UPSTREAM {proxyCount === 1 ? 'PROXY' : 'PROXIES'}
              </span>
            )}
          </div>
        </div>
