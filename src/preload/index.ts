...
const api = {
  ...
  // OOB Callback Server
  oobStart: () => ipcRenderer.invoke('oob:start'),
  oobStop: () => ipcRenderer.invoke('oob:stop'),
  oobStatus: () => ipcRenderer.invoke('oob:status'),
  oobGenerateId: () => ipcRenderer.invoke('oob:generateId'),
  onOobCallback: (callback: (data: any) => void) => {
    const listener = (_: any, data: any) => callback(data);
    ipcRenderer.on('oob:callback', listener);
    return () => ipcRenderer.removeListener('oob:callback', listener);
  },

  // Intruder
  intruderStart: (config: any) => ipcRenderer.invoke('intruder:start', config),
  intruderStop: () => ipcRenderer.invoke('intruder:stop'),
  intruderGetPayloads: () => ipcRenderer.invoke('intruder:payloads'),
  onIntruderResult: (callback: (data: any) => void) => {
    const listener = (_: any, data: any) => callback(data);
    ipcRenderer.on('intruder:result', listener);
    return () => ipcRenderer.removeListener('intruder:result', listener);
  },
