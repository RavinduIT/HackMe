...
import { expandEndpoint, extractPathParams, replacePathSegment } from './_params';
...
export class SqlInjectionModule implements ScanModuleInterface {
  ...
  async scan(url: string, endpoints: EndpointInfo[], client: HttpClient): Promise<ModuleFinding[]> {
    ...
    // Path parameter injection (numeric IDs in URLs like /users/123)
    for (const ep of endpoints) {
      for (const pp of extractPathParams(ep.url)) {
        const key = `pathparam:${ep.url}:${pp.paramIndex}`;
        if (tested.has(key)) continue;
        tested.add(key);
        for (const payload of ["'", "' OR '1'='1", "1 OR 1=1--"]) {
          const testUrl = replacePathSegment(ep.url, pp.paramIndex, payload);
          try {
            const res = await client.send(testUrl, { method: 'GET', timeout: 10000 });
            for (const { db, patterns } of ERROR_PATTERNS) {
              for (const pat of patterns) {
                if (pat.test(res.body)) {
                  findings.push({ severity: 'critical', title: `SQL Injection in URL Path (${db})`, description: `Path segment "${pp.original}" at ${ep.url} is injectable. ${db} error detected.`, url: ep.url, parameter: `path[${pp.paramIndex}]`, evidence: { payload, db, original: pp.original }, remediation: 'Use parameterized queries for all data lookups including URL path segments.', cwe_id: 'CWE-89', owasp_category: 'A03:2021', cvss_score: 9.8 });
                  break;
                }
              }
            }
