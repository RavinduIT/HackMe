  ...
  async scan(url: string, endpoints: EndpointInfo[], client: HttpClient): Promise<ModuleFinding[]> {
    ...
    for (const ep of jsonEndpoints) {
      ...
      for (const { body, desc } of payloads) {
        ...
        // Check if the pollution marker appears in the response body
        if (res.body.includes('hm_pp_test')) {
          findings.push({
            severity: 'high',
        ...
        // Follow-up: check if a clean request now shows altered behavior from pollution
        if (res.status === 200) {
          const cleanBody = ep.params.reduce((o: any, p) => { o[p] = 'test'; return o; }, {} as any);
          const followUp = await client.post(ep.url, JSON.stringify(cleanBody), { 'Content-Type': 'application/json' });
          if (followUp.body.includes('hm_pp_test') || followUp.body.includes('polluted')) {
            findings.push({
              severity: 'high',
              title: `Server-Side Prototype Pollution (${desc})`,
              description: `The endpoint "${ep.url}" is vulnerable to prototype pollution via ${desc}. After sending the pollution payload, a follow-up clean request shows altered behavior — the polluted property persists on the prototype.`,
              url: ep.url,
              remediation: 'Sanitize JSON input — reject or strip keys like __proto__, constructor, prototype. Use Object.create(null) for lookup maps. Use Map instead of plain objects.',
              cwe_id: 'CWE-1321', owasp_category: 'A03:2021', cvss_score: 7.5,
              evidence: { payload: desc, body: JSON.stringify(body), followup_confirmed: true },
            });
            break;
          }
        }
