...
import { getOobServer } from './oob-server';
...
export class ScanOrchestrator {
  ...
  async startScan(scanId: string): Promise<void> {
    ...
    // Start OOB callback server for blind vulnerability detection
    const oob = getOobServer();
    if (!oob.isRunning()) oob.start();
    ...
    try {
      ...
      const PROFILE_MODULES: Record<string, string[]> = {
        quick: ['recon', 'headers', 'tls-ssl'],
        standard: ['recon', 'headers', 'tls-ssl', 'sql-injection', 'xss', 'command-injection', 'ssti', 'nosql-injection', 'cors', 'open-redirect', 'csrf', 'clickjacking', 'jwt', 'access-control', 'cookie-analysis'],
        deep: ['recon', 'headers', 'tls-ssl', 'sql-injection', 'xss', 'command-injection', 'ssti', 'nosql-injection', 'cors', 'open-redirect', 'csrf', 'clickjacking', 'ssrf', 'xxe', 'jwt', 'access-control', 'crlf-injection', 'file-inclusion', 'host-header', 'cookie-analysis', 'js-analysis', 'graphql'],
        full: [],
      };
      let enabledModules: ScanModuleInterface[];
      if (config.modules && config.modules.length > 0) {
        enabledModules = ALL_MODULES.filter(m => config.modules.includes(m.id));
      } else {
        const profileMods = PROFILE_MODULES[config.profile || 'standard'];
        enabledModules = profileMods && profileMods.length > 0
          ? ALL_MODULES.filter(m => profileMods.includes(m.id))
          : ALL_MODULES;
      }
      ...
      // Wait for OOB callbacks from blind payloads
      if (oob.isRunning()) {
        this.emitProgress(scanId, { module: 'Checking blind callbacks...', percent: 97, requests: client.getRequestCount() });
        await new Promise(r => setTimeout(r, 5000));
        const oobResults = oob.getPendingResults().filter(r => r.received);
        for (const r of oobResults) {
          this.saveFinding(scanId, r.meta.module, {
            severity: 'critical', title: `Blind ${r.meta.title} (OOB Confirmed)`,
            description: `Out-of-band callback received from the target server, confirming a blind vulnerability. The server made an HTTP request to our callback server, proving it processes external input.`,
            url: r.meta.url, parameter: r.meta.payload,
            evidence: { callbackId: r.id, callbacks: r.callbacks },
            remediation: 'Sanitize and validate all user input. Never allow user-controlled data to reach network-level operations.',
            cwe_id: 'CWE-918', owasp_category: 'A10:2021', cvss_score: 9.8,
          });
        }
        if (oobResults.length > 0) console.log(`[Scanner] ${oobResults.length} blind vulnerabilities confirmed via OOB callbacks`);
      }

      const wasCancelled = !this.running.get(scanId);
      const counts = this.countFindings(scanId);
      const finalStatus = wasCancelled ? 'cancelled' : 'completed';
      this.db.prepare(
        'UPDATE scans SET status = ?, completed_at = CURRENT_TIMESTAMP, total_requests = ?, findings_count = ? WHERE id = ?'
      ).run(finalStatus, client.getRequestCount(), JSON.stringify(counts), scanId);
  ...
  stopScan(scanId: string) {
    ...
    this.emitProgress(scanId, { module: 'Cancelled', percent: 100, requests: 0 });
  }
